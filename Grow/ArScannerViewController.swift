...
class ArScannerViewController: UIViewController, ARSCNViewDelegate {
    ...
    func determineLogo (company: (String, String)) {
        ...
        if(Double(company.0)! > 0.85 && company.1 != "logo_none") {
            ...
            if(company.1 == "logo_apple") {
                companyInfo._companyLabel = "Apple Product"
                companyInfo._descriptionLabel = "Apple has been named one of the World’s Most Ethical Companies by the Ethisphere Institute."
            } else {
                companyInfo._companyLabel = "Starbucks Coffee"
                companyInfo._descriptionLabel = "Starbucks has been named one of the World’s Most Ethical Companies by the Ethisphere Institute."
            }
            companyInfo.updateContent()
        }
    }
}
